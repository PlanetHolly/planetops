...
## 2026-08-05 — **PR #44 OPEN**: the schedule now books to a PLAN line at 85% of the press day, and the last 15% is a visible RESERVE (Scheduler + Gauge) — https://github.com/PlanetHolly/planetops/pull/44
- Who:    Jean (via Claude, Opus). Branch `jean/plan-ceiling-85` off `origin/frontdoor-gate` in the schedule worktree, commit `e157efa`, authored PlanetHolly. Opened, NOT merged — awaiting Jean's go.
- Trigger: Rosa escalated 2026-08-03 that the floor feels it runs "200% every day". Jean's ruling that day — the estimator is accurate, the bar was set too high — is CONFIRMED by the numbers below.
- Grounded finding (measured 08-05 from `Press Calculator Auto` 596 rows / 361 with actuals + 71 Power-Scheduler snapshots + the Printavo exports). 🔴 **The "the team sprinted and it got baked into the estimates" theory is REFUTED:** bandana actual print pace was **0.278 min/pc pre-recurve** (n=26, median 0.308) vs **0.301 post-recurve** (n=85, median 0.310) — flat to slightly slower, no speed ratchet. Recurve-era jobs run at **89% of estimate** (median 0.86x, only 31% over), so the estimate is still slightly GENEROUS, not tight.
- Root cause: the OLD curve padded bandana print ~44% (est 0.400/pc vs measured ~0.29). **That pad WAS the slack.** The June recurve to 0.32 removed it — correctly — which freed **36.5 press-hours of schedule room across 178 jobs** (bandana -20%, apparel +35%, net -13%), and the room got filled. Same pace, more jobs, no recovery time between them.
- The arithmetic: an 8 h shift minus the three auto-deducted breaks = **7.0 workable press-hours (= the 420 already in the code)**. July scheduled 8.02 est-h/day ≈ **7.14 actual h/day = 102%**. Zero room for press checks, blanks, ink, screen swaps — none of which the estimate models. Imprints/day 6.6 (Jun) / 7.0 (Jul), matching the team's "6-8 projects" verbatim.
- What:   `capFor()` still returns the PHYSICAL day (420 std / 600 OT; per-day `store.caps` overrides still win). New `PLAN_PCT=0.85` + `planCap()` → **357 / 510** = the line we schedule to. `freeMin()` now measures against plan, so the ⚡ fits chip stops offering a day that is full by plan. **Nothing is hard-blocked** — drag/typed dates still reach reserve; a hard block would only teach us to raise the override and the ceiling would stop meaning anything. Three states → four: the old `70% of physical` amber fired BELOW the plan line and had stopped carrying information; **`reserve`** (plan → physical, violet) names the state the shop has been in since June with nothing on screen for it. Lane bars read `used/plan` + a tick at the plan line + `+N min into reserve` / `+N min over the day`; day headers total their overage; the range controls roll the week into **"N h over plan → ~N min OT per operator per day, or move a job"**. Calendar + day chips grade against plan too, so "100%" means the same thing on every surface.
- Gauge (`capacity/index.html`, PM-facing): same `PLAN_PCT`, new **Reserve** tier. 100% now means *booked to plan*, not *physically full*; Full still means past the physical day; "next open press day" stops offering a past-plan day. Legend + note rewritten to say a Reserve day still has minutes but they are spoken for.
- Bug avoided in passing: the cap editor parsed the displayed denominator (`sp.textContent.split('/')[1]`). With the readout now showing the plan, that would have written 85% of the day back AS the day on every edit. It reads `data-cap` instead.
- ⚠ Blast radius: **the Gauge is what PMs read to promise dates — re-basing it makes the shop look fuller and will push quoted turnarounds out.** That is the intended effect, but Kelly and the PM side will feel it the day it deploys. Jean chose to ship both halves together; **the heads-up to the PM side is Jean's to send and is NOT part of this PR.** Scheduler-side risk is low (sole user; revert = one constant).
- ⚠ Two data caveats carried forward: only **54-58% of recurve-era jobs have logged actuals** (plausibly biased toward calm jobs → measured pace may read optimistic; fix coverage before the next recurve), and the local Printavo exports are complete only **through April 2026** (May-Jul YoY needs live Printavo). Growth through April is **volume, not price**: orders +9-23% YoY but pieces +52-54%, pieces/order +26-40%, and $/piece FELL (Mar 7.11→6.20, Apr 6.31→5.81).
- Sizing the levers (July, the fullest complete month, 176.4 auto-press h / 22 days): at the 85% line the overflow is **1.33 press-h/day ≈ 40 min of OT per operator per day**. A second shift adds ~24 crew-h/day against that — **14-18x the need, wrong lever by an order of magnitude.** Do not build a second shift off this data.
- Proof:  `tests/test_plan_ceiling.js` (36, all pass): constants + both derived numbers; per-day override scaling (500 → 425); `freeMin` against plan (300 of 357 → 57 free, not 120 against physical); `overPlanMin`; the 4-state ladder at all four render sites + the dead amber gone; the editor `data-cap` fix; both apps agreeing on `PLAN_PCT`; **the real `laneBlock` markup** at each of the four states; **the real gauge `eff()`** at each band incl. OT scaling. Full regression suite green — **15 files, 213 assertions**, incl. `test_ot_daycap.js` (#43) whose 3-OT-site guard is untouched. Both pages' inline JS parse clean. Browser eyeball owed by Jean after deploy (a local static-server render check was attempted and Chrome refused both `127.0.0.1` and `localhost`, so the render is verified in Node against the shipped `laneBlock`/`eff`, not visually).
- Build doc updated?  no — this entry + the #44 body are the record.

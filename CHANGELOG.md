...
## 2026-07-22 — Outsourced packaging imprints now show in the QUEUE (✓ arrived no longer gates entry; the G1 vendor/in-house leg split still does) — **PR #28** open against `frontdoor-gate` — https://github.com/PlanetHolly/planetops/pull/28 (commit `7b8c6f5`, authored PlanetHolly per repo convention)
...
- Build doc updated?  no — this entry + the matching `Power_Scheduler/CHANGELOG.md` entry in Dropbox are the record. Committed as `7b8c6f5` on `jean/queue-outsourced-visibility` and opened as PR #28 against `frontdoor-gate` on 2026-07-22.

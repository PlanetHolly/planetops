...
- What:   `estimator/estimate.js` — new `manualCurve` {Bandana v:0.62, Apparel v:0.73} and `manualRules` {maxColors:2, inks:[plastisol, waterbase]}; `estimate()` takes `station`, and any station containing "manual" swaps the PRINT term to the manual curve. Setup, teardown and dry keep the auto formulas until manual setup is timed. Result carries `press:'manual'|'auto'`, `manualIssue` (text when the job is outside the rules — informational, never blocks), `provisional:true` on manual, and the derivation says "print MANUAL (provisional)". `mergeCfg` ignores unknown sheet keys, so the sheet feed cannot overwrite these until a manual row exists there.
- UI:     `estimator/index.html` — CSV import reads the `Station` column; manual rows get a purple **Manual\*** pill; jobs outside the rules get an amber ⚠ pill in Status (e.g. "⚠ 3 colors (max 2)", "⚠ discharge ink"); summary line counts "N on manual (M outside manual rules)"; Quick Estimate gets a **Press** selector; Formulas & Rates gets a MANUAL PRESS box with the curve + the rules; Save-to-Calculator payload now carries `station` (the /estimate webhook ignores unknown fields today). `schedule/index.html` — one line: the ⏱ chip passes `station:j.station`, so board chips on manual-station jobs use the manual curve too.
- Rules (Jean 2026-09-04, the floor's manual-press eligibility): apparel inside-neck prints always manual · apparel front/back 1–2 colors plastisol or waterbase · bandana 1–2 colors plastisol or waterbase · NO discharge (ink complexity) · NO 3+ colors (registration). The estimator FLAGS, it does not refuse — the schedule decides.
- Proof:  node harness — manual bandana 150 pcs → print 93 (actual was 100); manual 3-color tee → flagged "3 colors (max 2)"; manual discharge → flagged "discharge ink"; auto and no-station rows byte-identical to before. Headless render of the real 87-row export: 13 rows on manual, 1 flagged (27762-1, 3 colors), all pills/summary/quick-estimate/rates box present, zero page errors. Schedule inline script parses.
- ⚠ Not fixed here: manual actuals still land in the `Press Calculator Auto` tab and feed the AUTO recurve pool (the 8/25 "-24" sleeve prints were written as 124 min each from one batch session — they need clearing or marking before the next recurve). Manual jobs need their own home or a station flag in the calc before the manual curve can be refit from the sheet. Also still missing actuals: 27819-1 and 27690-1 were never logged in Printavo.
...
- 2 Minimum lead: `LEAD_DEFAULT=15` business days (Mon–Fri, no holiday calendar). Nothing inside the lead is offered as auto next-open; those cells are hatched + tagged "lead". Editable in Jean mode (black bar → "Minimum lead"), persisted as `overrides.__minLead` through the unchanged gauge webhook `1TNDY5XRyTm9E6OT` (it stores the overrides object verbatim, so no n8n change). Only non-default values are written. Bar verbiage: "We are booking N business days out · minimum lead 15 business days (date or later)". A manual pin inside the lead still wins but shows a red "Manual · inside the lead" tag (Jean mode only). Calendar depth 5 → 7 weeks (`WEEKS`) so the floor never falls off the page.
- 3 Disclaimer: green callout above the legend — only GREEN days are openings, yellow/red never, nothing inside the lead either. Legend relabeled: Limited/Full "· not an opening".
- 4 "Where does it fit?" (Jean mode only, read-only, writes nothing): loads `../estimator/estimate.js` (Recurve v2 + `loadRates()` live curve) and sizes product/qty/colors/ink/pallet × imprints, or takes entered minutes. Ranks Mon–Fri days in [earliest, must-print-before) by % after placement — "most room" (default) or "fill the day" — flags stays-green / goes-Limited, names the earliest day that stays green, warns when the pick is inside the lead, and proposes a two-day split across the roomiest days when nothing fits. Printed caveat: the gauge carries only minutes — changeover cap (4–5 projects/day), 1,800-unit wall, screen readiness, and the client ship date are NOT checked.
- Proof:  Playwright against the LIVE gauge JSON with POST intercepted (zero production writes): view mode hides Jean UI · iframe + top-URL param → Jean mode · sticky on bare URL · Exit → view · next open **Thu Sep 24 = 15 business days out** (Sep 24 is 69%, first green on/after the floor) · 14 lead cells hatched · 500-pc 2-color bandana = 193 min → Fri Sep 25 · split + impossible branches · lead=10 → floor Sep 17 · zero console errors. Screenshots in the session scratchpad.
- ⚠ Side finding, NOT fixed: the feed JSON still reports `capacity:600` / pct-off-600 on OT days (e.g. 08-31 `520 min · 87%`). The page ignores that and recomputes off its own `CAP.OT=525` (shows 99%), so PMs see the right number — but anything ELSE reading `gauge_load.pct` is 75 min/day optimistic. Feed `Utw3VU0cu8aKLTcf` still needs the 600 → 525 edit.
- Build doc updated?  no — this entry + the PR body are the record; `Gauge_System_How_It_Works.md` predates Jean mode, the lead, and the recommender.

...
- ⚠ KNOWN INCONSISTENCY, unresolved: `schedule/index.html:1018` carries a SECOND OT cap, `otCap:600` on the Auto Press lane, used by `capFor()` (:1229). It was left at 600 pending Jean's call. Until it is changed, the gauge and the scheduler board disagree about what an OT day holds. `tests/test_ot_daycap.js` asserts 600 but does so against its own hardcoded fixture, so it passes either way — the test will not catch the drift.
